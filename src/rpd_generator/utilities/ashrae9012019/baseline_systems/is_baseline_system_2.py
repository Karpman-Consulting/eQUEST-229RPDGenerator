...
def is_baseline_system_2(hvac, terminals_list, zones_list):
    """
...
def diagnose_baseline_system_2(hvac, terminals_list, zones_list):
    """
    Diagnostic wrapper for is_baseline_system_2 (PTHP).
    ...
    # Core system checks (order preserved from original logic)
    ...
    diagnostics["terminal_supplies_not_all_ducted"] = (
        not are_all_terminal_supplies_ducted(terminals_list)
    )
    diagnostics["terminal_types_cav"] = are_all_terminal_types_cav(terminals_list)

    passed = all(diagnostics.values())

    matched_system = HVAC_SYS.SYS_2 if passed else HVAC_SYS.UNMATCHED

    failed_checks = [name for name, ok in diagnostics.items() if not ok]

    return {
        "expected_system": "SYS_2",
