...
def get_zones_on_same_floor_list(rmd: dict, zone: dict) -> list[dict]:
    """
    ...
    zone: dict
    The zone for which we want to find all other zones on the same floor
    ...
    source_zone_floor_name = find_one("$.floor_name", zone)
    return find_all(
        f'$.buildings[*].building_segments[*].zones[*][?(@.floor_name="{source_zone_floor_name}")]',
        rmd,
    )
