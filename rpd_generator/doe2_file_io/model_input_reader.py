...
class ModelInputReader:
    ...
    def read_input_bdl_file(self, bdl_file_path: str) -> dict:
        ...
        with open(bdl_file_path, "r", encoding="cp1252", errors="replace") as bdl_file:
            doe2_version = None
            file_commands = {}
                ...
                # Parse the definition line and add the keyword and value to the active command dictionary
                    ...
                    for i in range(0, len(keywords_values), 2):
                        ...
                        if "(" in value and ")" not in value:
                            # Store the raw COEFFICIENTS data separate from the rounded DATA-FOR values
                            # Use the same key used by library values to ensure consistency.
                            multiline_key = key.replace("COEFFICIENTS", "COEF")
    ...
    @staticmethod
    def read_inp_files(inp_paths):
        normalized = [Path(p) for p in inp_paths]

        with tempfile.TemporaryDirectory() as tmp:
            tmp = Path(tmp)

            bdl_paths = []
            for inp_path in normalized:
            ...
            return [reader.read_input_bdl_file(path) for path in bdl_paths]
