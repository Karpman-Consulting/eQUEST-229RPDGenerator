...
def compare_json(
    reference, generated, optional_keys=[], tolerance_dict={}, warning_keys=[]
):
    warnings = []
    errors = []
    compare_dicts(
    ...
    for key in ref_dict:
        ...
        if isinstance(ref_dict[key], dict):
            ...
            compare_values(
                ref_dict[key],
                gen_dict[key],
                tolerance_dict.get(full_key, None),
                full_key,
                warnings,
                errors,
                warning_keys,
...
def compare_values(
    ref_value, gen_value, tolerance, key, warnings, errors, warning_keys
):
...
if __name__ == "__main__":
    if len(sys.argv) != 4:
        print(
            "Usage: python compare_json.py <reference_json> <generated_json> <tolerance_file>"
        )
        sys.exit(1)
    ...
    warnings, errors = compare_json(
        reference_json, generated_json, optional_keys, tolerance_values, warning_keys
    )
