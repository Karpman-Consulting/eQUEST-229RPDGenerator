...
from rpd_generator.utilities.jsonpath_utils import find_one, find_all, find_all_with_field_value, find_all_with_filters
    ...
    def get_best_match_attrs(target, candidates, attrs):
        ...
        for candidate in candidates:
            ...
            for attr in attrs:
                ...
                if attr == "azimuth":
                    if target_value == candidate_value or abs(target_value - candidate_value) == 180:
                        qty_matched += 1
                else:
...
def define_surface_map(generated_zone, reference_zone, generated_json, reference_json):
    ...
    for surface_name, filters in surface_types:
        generated_surfaces = find_all_with_filters("$.surfaces[*]", filters, generated_zone)
        reference_surfaces = find_all_with_filters("$.surfaces[*]", filters, reference_zone)

        if surface_name == "Interior Wall":
            # Extend with surfaces from other zones where this zone is the adjacent_zone
            generated_surfaces.extend(find_all_with_field_value(
                "$.ruleset_model_descriptions[0].buildings[0].building_segments[0].zones[*].surfaces[*]",
                "adjacent_zone", generated_zone_id, generated_json))
            reference_surfaces.extend(find_all_with_field_value(
                "$.ruleset_model_descriptions[0].buildings[0].building_segments[0].zones[*].surfaces[*]",
                "adjacent_zone", reference_zone.get("id"), reference_json))

        surface_map.update(match_surfaces(generated_zone_id, surface_name, generated_surfaces, reference_surfaces)[0])
...
def define_hvac_map(generated_zone, reference_zone, generated_json, reference_json):
    ...
    generated_hvacs = find_all("$.ruleset_model_descriptions[0].buildings[0].building_segments[0].heating_ventilating_air_conditioning_systems", generated_json)
    reference_hvacs = find_all("$.ruleset_model_descriptions[0].buildings[0].building_segments[0].heating_ventilating_air_conditioning_systems", reference_json)
...
def define_terminal_map(generated_zone, reference_zone, generated_json, reference_json):
    ...
    generated_terminals = find_all("$.ruleset_model_descriptions[0].buildings[0].building_segments[0].heating_ventilating_air_conditioning_systems[*].terminal_units[*]", generated_json)
    reference_terminals = find_all("$.ruleset_model_descriptions[0].buildings[0].building_segments[0].heating_ventilating_air_conditioning_systems[*].terminal_units[*]", reference_json)
...
def match_surfaces(zone_id, surface_type, generated_surfaces, reference_surfaces):
    ...
    if len(generated_surfaces) != len(reference_surfaces):
        ...
        local_surface_map[generated_surfaces[0].get("id")] = reference_surfaces[0].get("id")
        return local_surface_map, errors

    else:
        local_surface_map = find_matches_for("Surfaces", generated_surfaces, reference_surfaces)
        return local_surface_map, errors
...
def compare_json_files(test_case_name, spec_file, generated_json_file, reference_json_file):
    ...
    for i, generated_zone in enumerate(generated_zones):
        ...
        surface_map = define_surface_map(generated_zone, reference_zone, generated_json, reference_json)
        object_id_map.update(surface_map)

        hvac_map = define_hvac_map(generated_zone, reference_zone, generated_json, reference_json)
    ...
    for path_spec in json_test_key_paths:
        ...
        if json_key_path.split('.')[-1] == "design_electric_power":
            ...
            generated_supply_fan = find_all("$.ruleset_model_descriptions[*].buildings[*].building_segments[*].heating_ventilating_air_conditioning_systems[*].fan_system.supply_fans", generated_json)
        ...
        # Begin the General Comparison Methodology
            ...
            if "zones[*]" in json_key_path and "surfaces[*]" not in json_key_path and "terminals[*]" not in json_key_path:
                ...
                for generated_zone in generated_zones:
                    ...
                    aligned_generated_value = find_one(json_key_path.replace("zones[*]", f'zones[*][?(@.id="{generated_zone_id}")]'), generated_json)
                    if aligned_generated_value:
                        aligned_generated_values.append(aligned_generated_value)

                    aligned_reference_value = find_one(json_key_path.replace("zones[*]", f'zones[*][?(@.id="{reference_zone_id}")]'), reference_json)
                ...
                general_comparison_errors = compare_json_values(path_spec, aligned_generated_values, aligned_reference_values)
