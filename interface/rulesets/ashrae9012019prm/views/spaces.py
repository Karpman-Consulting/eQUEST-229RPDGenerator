...
from interface.rulesets.ashrae9012019prm.space_type_prediction_maps import *
...
class SpacesSubview(CTkXYFrame):
    ...
    def add_row(self, i, space_name):
        ...
        status_combo = None
        if not self.app_data.is_all_new_construction.get():
            self.app_data.status_vars[space_name] = ctk.StringVar()
            self.app_data.status_vars[space_name].trace_add(
                "write", lambda *_args: print(f"Status changed for {space_name}")
            )
            status_combo = ctk.CTkComboBox(
                self,
                values=self.app_data.StatusDescriptions,
                variable=self.app_data.status_vars[space_name],
                state=READONLY,
            )
        ...
        self.app_data.lighting_space_type_vars[space_name].trace_add(
            "write",
            lambda *_args, s=space_name: self.app_data.insert_to_rpd(
                self.app_data.LightingSpaceMapping2019ASHRAE901TG37, s
            ),
        )
        ...
        # Envelope
        self.app_data.envelope_space_type_vars[space_name] = ctk.StringVar()
        self.app_data.envelope_space_type_vars[space_name].trace_add(
            "write", lambda *_args: print(f"Envelope type changed for {space_name}")
        )
        envelope_space_type_combo = ctk.CTkComboBox(
            self,
            values=self.app_data.EnvelopeSpaceDescriptions2019ASHRAE901,
            variable=self.app_data.envelope_space_type_vars[space_name],
            state=READONLY,
        )
        ...
        # Ventilation
        self.app_data.ventilation_space_type_vars[space_name] = ctk.StringVar()
        self.app_data.ventilation_space_type_vars[space_name].trace_add(
            "write", lambda *_args: print(f"Ventilation type changed for {space_name}")
        )
        ventilation_space_type_combo = ctk.CTkComboBox(
            self,
            values=self.app_data.VentilationSpaceDescriptions2019ASHRAE901,
            variable=self.app_data.ventilation_space_type_vars[space_name],
            state=READONLY,
        )
        ...
        # SWH
        self.app_data.swh_space_type_vars[space_name] = ctk.StringVar()
        self.app_data.swh_space_type_vars[space_name].trace_add(
            "write", lambda *_args: print(f"SWH type changed for {space_name}")
        )
        swh_space_type_combo = ctk.CTkComboBox(
            self,
            values=self.app_data.ServiceWaterHeatingSpaceDescriptions2019ASHRAE901,
            variable=self.app_data.swh_space_type_vars[space_name],
            state=READONLY,
        )
        swh_space_type_combo._entry.configure(justify=LEFT)
        swh_space_type_combo.grid(row=(i + 1), column=5, padx=PAD20END, pady=PAD20END)

        # Lighting Occ Controls
        self.app_data.lighting_occ_controls_vars[space_name] = ctk.StringVar()
        self.app_data.lighting_occ_controls_vars[space_name].trace_add(
            "write",
            lambda *_args: print(f"Lighting occ controls changed for {space_name}"),
        )
        lighting_occ_controls_combo = ctk.CTkComboBox(
            self,
            values=self.app_data.LightingOccupancyControlDescriptions,
            variable=self.app_data.lighting_occ_controls_vars[space_name],
            state=READONLY,
        )
        ...
        self.app_data.daylighting_controls_vars[space_name] = ctk.StringVar()
        self.app_data.daylighting_controls_vars[space_name].trace_add(
            "write",
            lambda *_args: print(f"Daylighting controls changed for {space_name}"),
        )
        daylighting_controls_combo = ctk.CTkComboBox(
            self,
            values=self.app_data.LightingDaylightingControlDescriptions,
            variable=self.app_data.daylighting_controls_vars[space_name],
            state=READONLY,
        )
        ...
        # Occ Controls Modeled
        self.app_data.occ_controls_modeled_vars[space_name] = ctk.BooleanVar()
        self.app_data.occ_controls_modeled_vars[space_name].trace_add(
            "write",
            lambda *_args: print(f"Occ controls modeled changed for {space_name}"),
        )
        occ_controls_modeled_checkbox = ctk.CTkCheckBox(
            self,
            text="",
            variable=self.app_data.occ_controls_modeled_vars[space_name],
            width=30,
        )
        ...
        # Daylighting Modeled
        self.app_data.daylighting_modeled_vars[space_name] = ctk.BooleanVar()
        self.app_data.daylighting_modeled_vars[space_name].trace_add(
            "write",
            lambda *_args: print(f"Daylighting modeled changed for {space_name}"),
        )
        daylighting_modeled_checkbox = ctk.CTkCheckBox(
            self,
            text="",
            variable=self.app_data.daylighting_modeled_vars[space_name],
            width=30,
        )
    ...
    def set_subview_data(self):
        for row in self.widget_rows:
            ...
            space_name = name_label.cget("text")
            space_row_data = self.get_space_data(space_name)
            if not space_row_data:
                continue  # Skip if no data

            if status_combo and space_name in self.app_data.status_vars:
                self.app_data.status_vars[space_name].set(
                    space_row_data.get("Status", "")
                )
            self.app_data.lighting_space_type_vars[space_name].set(
                space_row_data.get("Lighting Space Type", "")
            )
            self.app_data.envelope_space_type_vars[space_name].set(
                space_row_data.get("Envelope Space Type", "")
            )
            self.app_data.ventilation_space_type_vars[space_name].set(
                space_row_data.get("Ventilation Space Type", "")
            )
            self.app_data.swh_space_type_vars[space_name].set(
                space_row_data.get("SWH Space Type", "")
            )
            self.app_data.lighting_occ_controls_vars[space_name].set(
                space_row_data.get("Lighting Occ. Controls", "")
            )
            self.app_data.daylighting_controls_vars[space_name].set(
                space_row_data.get("Daylighting Controls", "")
            )
            self.app_data.occ_controls_modeled_vars[space_name].set(
                space_row_data.get("Occ. Controls Modeled", False)
            )
            self.app_data.daylighting_modeled_vars[space_name].set(
                space_row_data.get("Daylighting Modeled", False)
            )

    def get_space_data(self, space_name):
        """Helper method to get space data from the app_data for a specific space."""
        spaces_data = self.app_data.interface_data.get(self.json_representation, [])
    ...
    def calculate_space_areas(space_area, lpd_1, lpd_2, lighting_power):
        """
        Calculate the areas of two spaces based on the space area, lighting power density (LPD) values, and total lighting power.
        """
        # X + Y = Space Area
    ...
    def calculate_value_proportionate_to_lpd(
        ...
        """
        Calculate the value proportionate to the lighting power density for any equations that follows this pattern:
    ...
    def predict_space_type_from_substring(space):
        """
        If the C-ACTIVITY-DESC for a space is a substring of any space type, return the space type with the lowest code
        as the predicted space type. Else return None.
        """
    ...
    def predict_space_type_from_lpd(space):
        """
        If the LPD for a space matches any of the LPDs in the SPACE_TYPE_LPD dictionary, return the space type with the
    ...
    def assign_new_space_data(self):
        """
        For every new space we created during the prediction, either add a default new space, or a new space with the
        calculated data values. Do this for every RMD. When we have a situation with more than 2 type codes, there are
        (n-1) new spaces created that already have their '_#' suffixes. What is stored in their 'data' field in the
        new_spaces dictionary is the original space name that we need to copy over the original space data. If we have a
        situation with only 2 type codes, we create a new space with the original space name and the 'data' is actually
